...
def get_listb(page = 0)
  JSON.parse(RestClient.get("https://api.mercadolibre.com/sites/MLB/search?category=MLB3007&condition=used&sort=price_desc&price=100-400&offset=#{ page * 50 }"))["results"].map do |e|
    OpenStruct.new(link: e["permalink"], image: e["thumbnail"], title: e["title"], price: e["price"], seller: e["seller"]["id"])
  end.select do |e|
    !($blacklist.any? { |b| e.link.include?(b) } || $sellers.include?(e.seller))
  end
end

def get_list(page = 0)
  results = get_listb(page)
  empties = 0
  while results.length < 20 do
    page = page + 1
    res = get_listb(page)
    empties = empties + 1 if res.length == 0
    break if empties > 10
    results = results + res
    
    puts "#{results.length} so far"
  end
  return OpenStruct.new(r: results, page: page + 1)
end

get '/' do
  @r = get_list(params[:page].to_i || 0)
  erb :index
end
